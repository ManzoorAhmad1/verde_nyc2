...
export default function MenuClient({ pageData }: Props) {
  ...
  const introSection = textSections.find((s: PageSection) => s.heading?.includes('CULINARY') || s.heading?.includes('EXCELLENCE')) || null;
  const ctaSection = textSections.find((s: PageSection) => s.heading?.includes('RESERVE') || s.ctaText) || null;
  const menuCategories = sections
    .filter((s: PageSection) => s.type === 'menu-category')
    .map((s: PageSection) => ({
      title: s.heading,
      description: s.content,
      image: s.images && s.images.length > 0 ? s.images[0] : '',
      menuImages: s.images && s.images.length > 1 ? s.images.slice(1) : [],
    }));
  ...
  // Fetch image URL -> base64 string via fetch
  ...
  // PDF Download using jsPDF loaded from CDN
  ...
  return (
    <>
      <div className="menu-page">
            ...
            {menuCategories.map((category, index) => (
              <div key={index} className="menu-category-card">
                ...
                <div className="menu-category-content">
                  ...
                  <button onClick={() => openMenu(category)} className="menu-view-btn">
                    VIEW MENU
                  </button>
                </div>
              </div>
            ...
            >
              ...
              >
                ...
                {selectedMenu.images.length > 1 && (
                  <>
                    <button className="modal-arrow modal-arrow-left" onClick={handlePrev} disabled={isAnimating} aria-label="Previous image">
                      <svg viewBox="0 0 24 24" width="18" height="18" stroke="currentColor" strokeWidth="2.5" fill="none">
                        <path d="M15 18l-6-6 6-6" />
                      </svg>
                    </button>
                    <button className="modal-arrow modal-arrow-right" onClick={handleNext} disabled={isAnimating} aria-label="Next image">
                      <svg viewBox="0 0 24 24" width="18" height="18" stroke="currentColor" strokeWidth="2.5" fill="none">
                  ...
                  <div className="dot-container">
                    {selectedMenu.images.map((_, index) => (
                      <button
                        ...
                        onClick={() => { setCurrentImageIndex(index); setIsZoomed(false); resetScroll(); }}
        ...
        @media (max-width: 768px) {
          .modal-header { padding: 4px 12px; min-height: 40px; }
        }

        @media (max-width: 480px) {
          .modal-header { padding: 4px 10px; min-height: 38px; }
        }
        ...
        @media (max-width: 768px) {
          .modal-label { font-size: 10px; padding: 2px 8px; }
        }

        @media (max-width: 480px) {
          .modal-label { font-size: 9px; padding: 2px 6px; }
        }
        ...
        @media (max-width: 768px) {
          .modal-icon-btn, .modal-close-btn { width: 32px; height: 32px; }
        }

        @media (max-width: 480px) {
          .modal-icon-btn, .modal-close-btn { width: 30px; height: 30px; }
        }
        ...
        .modal-arrow:disabled { opacity: 0.4; cursor: not-allowed; }
        ...
        @media (max-width: 480px) {
          .modal-arrow { width: 32px; height: 32px; }
        }
        ...
        .modal-scroll-box::-webkit-scrollbar { width: 6px; }
        .modal-scroll-box::-webkit-scrollbar-track { background: #f1f5f9; }
        .modal-scroll-box::-webkit-scrollbar-thumb { background: #cbd5e0; border-radius: 8px; }
        .modal-scroll-box::-webkit-scrollbar-thumb:hover { background: #8e402f; }

        @media (max-width: 1023px) and (min-width: 769px) {
          .modal-scroll-box { padding: 16px; }
        }

        @media (max-width: 768px) {
          .modal-scroll-box { padding: 8px 4px; }
        }

        @media (max-width: 480px) {
          .modal-scroll-box { padding: 4px 2px; }
        }
        ...
        @media (max-width: 768px) {
          .modal-image { max-width: 100%; width: 100%; border-radius: 2px; }
        }

        .modal-image.zoomed { max-width: 100%; width: 100%; cursor: zoom-out; }
        .modal-image.fade { opacity: 0.3; }
        ...
        @media (max-width: 768px) {
          .scroll-hint { font-size: 11px; margin-top: 12px; padding-bottom: 8px; }
        }

        @media (max-width: 480px) {
          .scroll-hint { font-size: 10px; margin-top: 8px; padding-bottom: 4px; }
        }
        ...
        @media (max-width: 768px) {
          .modal-bottom-bar { padding: 6px 12px; gap: 12px; min-height: 38px; }
        }

        @media (max-width: 480px) {
          .modal-bottom-bar { padding: 4px 10px; gap: 10px; min-height: 36px; }
        }
        ...
        @media (max-width: 768px) {
          .page-indicator { font-size: 12px; min-width: 45px; }
        }

        @media (max-width: 480px) {
          .page-indicator { font-size: 11px; min-width: 40px; }
        }
        ...
        .modal-download-btn:hover:not(:disabled) { background: #7a3526; border-color: #7a3526; }
        .modal-download-btn:disabled { opacity: 0.7; cursor: not-allowed; }

        @media (max-width: 768px) {
          .modal-download-btn { height: 32px; padding: 0 10px; font-size: 11px; }
          .download-btn-text { display: inline; }
        }

        @media (max-width: 480px) {
          .modal-download-btn { height: 30px; padding: 0 8px; font-size: 10px; gap: 4px; }
          .download-btn-text { display: inline; }
        }

        @media (max-width: 340px) {
          .download-btn-text { display: none; }
          .modal-download-btn { width: 30px; padding: 0; justify-content: center; }
        }
